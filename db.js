...
const HAS_SQLITE = fs.existsSync(SQLITE);
let memoryState = null;
const memoryTokens = new Map();

function sql(query) { return execFileSync(SQLITE, [DB_FILE, query], { encoding: 'utf8' }).trim(); }
function init(seed) {
  if (!HAS_SQLITE) { try { memoryState = JSON.parse(fs.readFileSync(LEGACY_FILE, 'utf8')); } catch { memoryState = structuredClone(seed); } return; }
...
function saveOAuthToken(userId, provider, token, key) { if (!HAS_SQLITE) { memoryTokens.set(`${userId}:${provider}`, token); return; } const encrypted = encryptToken(token, key).replace(/'/g, "''"); sql(`INSERT OR REPLACE INTO oauth_tokens VALUES (${q(userId)}, ${q(provider)}, '${encrypted}', datetime('now'));`); }
function getOAuthToken(userId, provider, key) { if (!HAS_SQLITE) return memoryTokens.get(`${userId}:${provider}`) || null; const row = sql(`SELECT encrypted_token FROM oauth_tokens WHERE user_id = ${q(userId)} AND provider = ${q(provider)};`); return row ? decryptToken(row, key) : null; }
function read() { return HAS_SQLITE ? JSON.parse(sql("SELECT state_json FROM app_state WHERE id = 1;")) : structuredClone(memoryState); }
...
function write(state) { if (!HAS_SQLITE) { memoryState = structuredClone(state); return; } const json = JSON.stringify(state).replace(/'/g, "''"); sql(`UPDATE app_state SET state_json = '${json}', updated_at = datetime('now') WHERE id = 1;`); normalize(state); }
